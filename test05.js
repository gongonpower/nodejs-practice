...
JSONLineReader.prototype._read = function(size){
    ...
    if (lineIndex !== -1) {
        ...
        if (line) {
            ...
            console.log('before buffer:', this._buffer)
            this._buffer = this._buffer.slice()
            console.log('after buffer:', this._buffer)
        }
    }
}
...
jsonLineReader.on('readable', function(){
    var output = jsonLineReader.read()
